...
impl Erc1155Storage {
    ...
    fn parse_and_store_attributes(
        ...
        for (key, value) in obj {
            ...
            stmt.execute(params![&token_blob, &token_id_blob, key, &value_str])?;
        }
        ...
        if let Some(attrs) = obj.get("attributes").and_then(|v| v.as_array()) {
            for attr in attrs {
                ...
                stmt.execute(params![&token_blob, &token_id_blob, key, &value])?;
            }
        }
        ...
        if let Some(props) = obj.get("properties").and_then(|v| v.as_object()) {
            for (key, value) in props {
                let value_str = match value {
                    serde_json::Value::String(s) => s.clone(),
                    serde_json::Value::Object(o) => match o.get("value") {
                        Some(serde_json::Value::String(s)) => s.clone(),
                        Some(serde_json::Value::Null) | None => continue,
                        Some(other) => other.to_string(),
                    },
