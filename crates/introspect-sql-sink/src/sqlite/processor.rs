...
use sqlx::{Error as SqlxError, Row, Sqlite};
...
impl<T: DbConnection<Sqlite>> DbConnection<Sqlite> for IntrospectSqliteDb<T> {
    fn pool(&self) -> &sqlx::SqlitePool {
        self.pool.pool()
    }
}

impl<T: DbConnection<Sqlite> + Send + Sync> IntrospectSqliteDb<T> {
