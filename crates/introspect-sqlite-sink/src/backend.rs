...
use torii_introspect_sql_sink::processor::IntrospectDb;
use torii_introspect_sql_sink::{
    DbColumn, DbDeadField, DbResult, DbTable, DeadField, IntrospectBackend, IntrospectSqlSink,
    RecordResult, Table, TableResult,
};
use torii_sql::{DbConnection, Queries, Sqlite, SqlitePool, SqliteQuery};

use crate::query::create_table_query;
...
#[async_trait]
impl<T: DbConnection<Sqlite> + Send + Sync> IntrospectBackend for SqliteBackend<T> {
    type DB = Sqlite;
