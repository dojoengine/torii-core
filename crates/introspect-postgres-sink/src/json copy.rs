...
impl CairoSerializer for PostgresJsonSerializer {
    fn serialize_byte_array<S: serde::Serializer>(
        &self,
        serializer: S,
        value: &[u8],
    ) -> Result<S::Ok, S::Error> {
    ...
    fn serialize_felt<S: serde::Serializer>(
        &self,
        serializer: S,
        value: &[u8; 32],
    ) -> Result<S::Ok, S::Error> {
    ...
    fn serialize_eth_address<S: serde::Serializer>(
        &self,
        serializer: S,
        value: &[u8; 20],
    ) -> Result<S::Ok, S::Error> {
    ...
    fn serialize_enum<'a, S: serde::Serializer, D: introspect_types::CairoDeserializer>(
        &'a self,
        serializer: S,
        variant_name: &str,
        type_def: &'a introspect_types::TypeDef,
        de: &'a std::cell::RefCell<&'a mut D>,
    ) -> Result<S::Ok, S::Error>
    where
        Self: Sized,
    {
