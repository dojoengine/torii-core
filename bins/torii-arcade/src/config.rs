use anyhow::{bail, Result};
use clap::{Parser, ValueEnum};
use starknet::core::types::Felt;
use std::path::Path;

const DEFAULT_RPC_URL: &str = "https://api.cartridge.gg/x/starknet/mainnet";
const DEFAULT_WORLD_ADDRESS: &str =
    "0x07a079295990e43441a7389fdc3b9ba063c6cd6aee16fb846f598c42a9f04ff7";

const DEFAULT_DOJO_CONTRACTS: &[&str] = &[
    "0x07a079295990e43441a7389fdc3b9ba063c6cd6aee16fb846f598c42a9f04ff7",
    "0x5c6d0020a9927edca9ddc984b97305439c0b32a1ec8d3f0eaf6291074cc9799",
    "0x04f3dccb47477c087ad9c76b8067b8aadded57f8df7f2d7543e6066bcb25332c",
    "0x6a9e4c6f0799160ea8ddc43ff982a5f83d7f633e9732ce42701de1288ff705f",
    "0x33e7c93e90e9f35e1daadb3ab1c59cca5944deff5a79e6167f587f13efe18cc",
    "0x53e0ba5aa2cc9cf3b21ffaf0d0e9407eab627b759404e35e69e8d3e6f1e3616",
    "0x8b4838140a3cbd36ebe64d4b5aaf56a30cc3753c928a79338bf56c53f506c5",
    "0x628b6af861259ce9ce22e8df6ebd5fbf4fde9a7d4e6eba086aa8dd32325f3b0",
    "0x76b1269937debcb70fb5f59dea1c6f9ce24c9415b23876f623385933a6d2681",
    "0x30d5d5c610dd736faea146b20b850af64e34ca6e5c5a66462f76f32f48dd997",
];

const DEFAULT_ERC1155_CONTRACTS: &[&str] =
    &["0x048089694913599fb5e23e3e9ee01287c5ea9a54d7d48b08cbbf0091995d502e"];

const DEFAULT_ERC721_CONTRACTS: &[&str] = &[
    "0x1e1c477f2ef896fd638b50caa31e3aa8f504d5c6cb3c09c99cd0b72523f07f7",
    "0x7ae27a31bb6526e3de9cf02f081f6ce0615ac12a6d7b85ee58b8ad7947a2809",
    "0x60e8836acbebb535dfcd237ff01f20be503aae407b67bb6e3b5869afae97156",
    "0x7ad410c472c1d61ce318dd617a479c977c85275afbf7991a1e1461ffe626a3d",
    "0x572fe34a769058c62a66f0c4854d08ccfd21fcbb4f0b1685a1868b84c6ee266",
    "0x051d0844f96f86c7363cc7eb3ab939e0ef5b70939dcbc17895b2fa178d9af420",
    "0x0537fa10cefbb8ff7e61e86e950746809f95faa7398f250f0063069b29bb7933",
    "0x2e9c711b1a7e2784570b1bda5082a92606044e836ba392d2b977d280fb74b3c",
    "0x7aaa9866750a0db82a54ba8674c38620fa2f967d2fbb31133def48e0527c87f",
    "0x71333ac75b7d5ba89a2d0c2b67d5b955258a4d46eb42f3428da6137bbbfdfd9",
    "0x14aa76e6c6f11e3f657ee2c213a62006c78ff2c6f8ed40b92c42fd554c246f2",
    "0x0413485dbeccec6320d35dcba14375e7ca973f021486496a61286edb958fd861",
    "0x04fa864a706e3403fd17ac8df307f22eafa21b778b73353abf69a622e47a2003",
    "0x02acee8c430f62333cf0e0e7a94b2347b5513b4c25f699461dd8d7b23c072478",
    "0x00539f522b29ae9251dbf7443c7a950cf260372e69efab3710a11bf17a9599f1",
    "0x04f5e296c805126637552cf3930e857f380e7c078e8f00696de4fc8545356b1d",
    "0x0158160018d590d93528995b340260e65aedd76d28a686e9daa5c4e8fad0c5dd",
];

const DEFAULT_ERC20_CONTRACTS: &[&str] = &[
    "0x0410466536b5ae074f7fea81e5533b8134a9fa08b3dd077dd9db08f64997d113",
    "0x0124aeb495b947201f5fac96fd1138e326ad86195b98df6dec9009158a533b49",
    "0x264be95a4a2ace20add68cb321acdccd2f9f8440ee1c7abd85da44ddab01085",
    "0x0695b08ecdfdd828c2e6267da62f59e6d7543e690ef56a484df25c8566b332a5",
    "0x0ac965f9e67164723c16735a9da8dbc9eb8e43b1bd0323591e87c056badf606",
    "0x67e4ac00a241be06ba6afc11fa2715ec7da0c42c05a67ef6ecfcfeda725aaa8",
    "0x3bc86299bee061c7c8d7546ccb62b9daf9bffc653b1508facb722c6593874bc",
    "0x40d8907cec0f7ae9c364dfb12485a1314d84c129bf1898d2f3d4b7fcc7d44f4",
    "0x439a1c010e3e1bb2d43d43411000893c0042bd88f6c701611a0ea914d426da4",
    "0x0ce635e3f241b0ae78c46a929d84a9101910188f9c4024eaa7559556503c31a",
    "0x66ed5c928ee027a9419ace1cbea8389885161db5572a7c5c4fef2310e9bf494",
    "0x3b6448d09dcd023507376402686261f5d6739455fa02f804907b066e488da66",
    "0x6fe21d2d4a8a05bdb70f09c9250af9870020d5dcc35f410b4a39d6605c3e353",
    "0x1720cf6318bff45e62acc588680ae3cd4d5f8465b1d52cb710533c9299b031a",
    "0x555d713e59d4ff96b7960447e9bc9e79bfdeab5b0eea74e3df81bce61cfbc77",
    "0x0dff9dca192609c4e86ab3be22c7ec1e968876c992d21986f3c542be97fa2f",
    "0x5620aa7170cd66dbcbc37d03087bfe4633ffef91d3e4d97b501de906004f79b",
    "0x0e03ea8ae385f64754820af5c01c36abf1b8130dd6797d3fd9d430e4114e876",
    "0x2f8dd022568af8f9f718aa37707a9b858529db56910633a160456838b6cbcbc",
    "0x3d9b66720959d0e7687b898292c10e62e78626f2dba5e1909961a2ce3f86612",
    "0x1d655ac834d38df7921074fc1588411e202b1af83307cbd996983aff52db3a8",
    "0x625c1f789b03ebebc7a9322366f38ebad1f693b84b2abd8cb8f5b2748b0cdd5",
    "0x68b6e23cbbd58a644700f55e96c83580921e9f521b6e5175396b53ba7910e7d",
    "0x4485f5a6e16562e1c761cd348e63256d00389e3ddf4f5d98afe7ab44c57c481",
    "0x35e24c02409c3cfe8d5646399a62c4d102bb782938d5f5180e92c9c62d3faf7",
    "0x3956a5301e99522038a2e7dcb9c2a89bf087ffa79310ee0a508b5538efd8ddd",
    "0x367f838f85a2f5e1580d6f011e4476f581083314cff8721ba3dda9706076eed",
    "0x67ba235c569c23877064b2ac6ebd4d79f32d3c00f5fab8e28a3b5700b957f6",
    "0x3bf856515bece3c93f5061b7941b8645f817a0acab93c758b8c7b4bc0afa3c6",
    "0x57a3f1ee475e072ce3be41785c0e889b7295d7a0dcc22b992c5b9408dbeb280",
    "0x27719173cfe10f1aa38d2aaed0a075b6077290f1e817aa3485d2b828394f4d9",
    "0x01bfe97d729138fc7c2d93c77d6d1d8a24708d5060608017d9b384adf38f04c7",
    "0x2549653a4ae1ff8d04a20b8820a49cbe97486c536ec0e4c8f68aa33d80067cf",
    "0x68a7a07e08fc3e723a878223d00f669106780d5ea6665eb15d893476d47bf3b",
    "0x05735fa6be5dd248350866644c0a137e571f9d637bb4db6532ddd63a95854b58",
    "0x0415c058a41cc80e7368562564c96fc4e3c03b23e32ba07a5c8cadc262b50c3c",
    "0x04b66d22d3001daad50fb853c0c1cb3b96d1745acb295bae4a6d54b29125ed09",
    "0x01920ef3c5e765454dd3f6aeb5420ef524830e0b5f9a95ec2e1b9ee2073a16d1",
    "0x079aba4c89e9cc3495318d2479fe93601e1188ff5d9a8823e3dc736d74bb437f",
    "0x038217779933c147320af3239e2dd098312e3074e0898001c79939c2e676fe8c",
    "0x04c090a1a34a3ba423e63a498ce23de7c7a4f0f1a8128fa768a09738606cbb9e",
    "0x056893df1e063190aabda3c71304e9842a1b3d638134253dd0f69806a4f106eb",
    "0x040025cec149bf1f58d2e34a6924605b571a5fce7b798a47ec52cfbd3ff68b6e",
    "0x078c1138aa1cfd27436b26279d5ac4e3f8f5a432927d85d22b2a2e7c0e5528b4",
    "0x074ad80778e07102902abdec71e0161023b45d1204c29e2c4ec3befab3bb82f5",
    "0x00dcdc180a8b4b9cef2d039462ad30de95c5609178a1c2bc55779309c07d45db",
];

#[derive(Clone, Debug, ValueEnum, PartialEq, Eq)]
pub enum MetadataMode {
    Inline,
    Deferred,
}

#[derive(Parser, Debug)]
#[command(name = "torii-arcade")]
#[command(
    about = "Arcade-dedicated Torii backend with Dojo introspect, ECS, and token services",
    long_about = None
)]
pub struct Config {
    #[arg(long, env = "STARKNET_RPC_URL", default_value = DEFAULT_RPC_URL)]
    pub rpc_url: String,

    #[arg(long, env = "ARCADE_WORLD_ADDRESS", default_value = DEFAULT_WORLD_ADDRESS)]
    pub world_address: String,

    #[arg(long, value_delimiter = ',', default_values = DEFAULT_DOJO_CONTRACTS)]
    pub dojo_contracts: Vec<String>,

    #[arg(long, value_delimiter = ',')]
    pub introspect_contracts: Option<Vec<String>>,

    #[arg(long, default_value = "0")]
    pub from_block: u64,

    #[arg(long)]
    pub to_block: Option<u64>,

    #[arg(long, default_value = "./torii-data")]
    pub db_dir: String,

    #[arg(long, env = "DATABASE_URL")]
    pub database_url: Option<String>,

    #[arg(long, env = "STORAGE_DATABASE_URL")]
    pub storage_database_url: Option<String>,

    #[arg(long, env = "TOKEN_STORAGE_DATABASE_URL")]
    pub token_storage_database_url: Option<String>,

    #[arg(long, default_value = "3000")]
    pub port: u16,

    #[arg(long)]
    pub observability: bool,

    #[arg(long, visible_alias = "chunk-size", default_value = "1000")]
    pub event_chunk_size: u64,

    #[arg(long, visible_alias = "batch-size", default_value = "10000")]
    pub event_block_batch_size: u64,

    #[arg(long, default_value = "2")]
    pub max_prefetch_batches: usize,

    #[arg(long, default_value = "0")]
    pub rpc_parallelism: usize,

    #[arg(long)]
    pub max_db_connections: Option<u32>,

    #[arg(long)]
    pub ignore_saved_state: bool,

    #[arg(long)]
    pub allow_unsafe_latest_schema_bootstrap: bool,

    #[arg(long, value_enum, default_value = "deferred")]
    pub metadata_mode: MetadataMode,

    #[arg(long, default_value_t = false, action = clap::ArgAction::Set)]
    pub include_well_known: bool,

    #[arg(long, value_delimiter = ',', default_values = DEFAULT_ERC721_CONTRACTS)]
    pub erc721: Vec<String>,

    #[arg(long, value_delimiter = ',', default_values = DEFAULT_ERC20_CONTRACTS)]
    pub erc20: Vec<String>,

    #[arg(long, value_delimiter = ',', default_values = DEFAULT_ERC1155_CONTRACTS)]
    pub erc1155: Vec<String>,
}

impl Config {
    pub fn parse_address(addr: &str, field: &str) -> Result<Felt> {
        Felt::from_hex(addr).map_err(|e| anyhow::anyhow!("Invalid {field} {addr}: {e}"))
    }

    pub fn world_address(&self) -> Result<Felt> {
        Self::parse_address(&self.world_address, "world address")
    }

    pub fn dojo_event_contract_addresses(&self) -> Result<Vec<Felt>> {
        let mut contracts = Vec::with_capacity(self.dojo_contracts.len() + 1);
        push_unique_felt(&mut contracts, self.world_address()?);
        for contract in &self.dojo_contracts {
            push_unique_felt(
                &mut contracts,
                Self::parse_address(contract, "Dojo contract")?,
            );
        }
        Ok(contracts)
    }

    pub fn introspect_contract_addresses(&self) -> Result<Vec<Felt>> {
        let source = self
            .introspect_contracts
            .as_ref()
            .unwrap_or(&self.dojo_contracts);
        parse_unique_addresses(source, "introspect contract")
    }

    pub fn erc20_addresses(&self) -> Result<Vec<Felt>> {
        parse_unique_addresses(&self.erc20, "ERC20 contract")
    }

    pub fn erc721_addresses(&self) -> Result<Vec<Felt>> {
        parse_unique_addresses(&self.erc721, "ERC721 contract")
    }

    pub fn erc1155_addresses(&self) -> Result<Vec<Felt>> {
        parse_unique_addresses(&self.erc1155, "ERC1155 contract")
    }

    pub fn well_known_erc20_contracts() -> Vec<(Felt, &'static str)> {
        vec![
            (
                Felt::from_hex_unchecked(
                    "0x049D36570D4e46f48e99674bd3fcc84644DdD6b96F7C741B1562B82f9e004dC7",
                ),
                "ETH",
            ),
            (
                Felt::from_hex_unchecked(
                    "0x04718f5a0Fc34cC1AF16A1cdee98fFB20C31f5cD61D6Ab07201858f4287c938D",
                ),
                "STRK",
            ),
        ]
    }

    pub fn engine_database_url(&self) -> String {
        if let Some(database_url) = &self.database_url {
            return database_url.clone();
        }

        if let Some(storage_database_url) = &self.storage_database_url {
            if is_postgres_url(storage_database_url) {
                return storage_database_url.clone();
            }
        }

        self.storage_database_url
            .as_ref()
            .filter(|url| *url == ":memory:" || url.starts_with("sqlite::memory:"))
            .cloned()
            .unwrap_or_else(|| {
                Path::new(&self.db_dir)
                    .join("arcade-engine.db")
                    .to_string_lossy()
                    .to_string()
            })
    }

    pub fn storage_database_url(&self) -> Result<String> {
        let url = self.storage_database_url.clone().unwrap_or_else(|| {
            if let Some(database_url) = &self.database_url {
                if is_postgres_url(database_url) {
                    return database_url.clone();
                }
            }

            Path::new(&self.db_dir)
                .join("arcade-introspect.db")
                .to_string_lossy()
                .to_string()
        });

        validate_database_url("--storage-database-url", &url)?;
        Ok(url)
    }

    pub fn token_storage_urls(&self) -> Result<(String, String, String)> {
        if let Some(url) = &self.token_storage_database_url {
            validate_database_url("--token-storage-database-url", url)?;
            return Ok((url.clone(), url.clone(), url.clone()));
        }

        if let Some(url) = &self.storage_database_url {
            if is_postgres_url(url) {
                return Ok((url.clone(), url.clone(), url.clone()));
            }
        }

        if let Some(url) = &self.database_url {
            if is_postgres_url(url) {
                return Ok((url.clone(), url.clone(), url.clone()));
            }
        }

        Ok((
            Path::new(&self.db_dir)
                .join("arcade-erc20.db")
                .to_string_lossy()
                .to_string(),
            Path::new(&self.db_dir)
                .join("arcade-erc721.db")
                .to_string_lossy()
                .to_string(),
            Path::new(&self.db_dir)
                .join("arcade-erc1155.db")
                .to_string_lossy()
                .to_string(),
        ))
    }
}

fn is_postgres_url(url: &str) -> bool {
    url.starts_with("postgres://") || url.starts_with("postgresql://")
}

fn validate_database_url(flag: &str, url: &str) -> Result<()> {
    if is_postgres_url(url)
        || url.starts_with("sqlite:")
        || url == ":memory:"
        || !url.contains("://")
    {
        Ok(())
    } else {
        bail!("{flag} must be a PostgreSQL or SQLite URL/path")
    }
}

fn parse_unique_addresses(addresses: &[String], field: &str) -> Result<Vec<Felt>> {
    let mut parsed = Vec::with_capacity(addresses.len());
    for address in addresses {
        let felt = Config::parse_address(address, field)?;
        push_unique_felt(&mut parsed, felt);
    }
    Ok(parsed)
}

fn push_unique_felt(target: &mut Vec<Felt>, value: Felt) {
    if !target.contains(&value) {
        target.push(value);
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use clap::Parser;

    #[test]
    fn defaults_include_arcade_world_and_seed_collection() {
        let cfg = Config::parse_from(["torii-arcade"]);
        let dojo = cfg.dojo_event_contract_addresses().unwrap();
        let introspect = cfg.introspect_contract_addresses().unwrap();
        let erc721 = cfg.erc721_addresses().unwrap();
        let erc20 = cfg.erc20_addresses().unwrap();
        let erc1155 = cfg.erc1155_addresses().unwrap();

        assert_eq!(
            dojo[0],
            Felt::from_hex_unchecked(
                "0x2d26295d6c541d64740e1ae56abc079b82b22c35ab83985ef8bd15dc0f9edfb"
            )
        );
        assert_eq!(dojo.len(), 10);
        assert_eq!(introspect.len(), 9);
        assert!(!introspect.contains(&Felt::from_hex_unchecked(
            "0x2d26295d6c541d64740e1ae56abc079b82b22c35ab83985ef8bd15dc0f9edfb"
        )));
        assert_eq!(erc721.len(), 17);
        assert_eq!(erc20.len(), 46);
        assert_eq!(erc1155.len(), 1);
    }

    #[test]
    fn explicit_introspect_contracts_override_defaults() {
        let cfg = Config::parse_from([
            "torii-arcade",
            "--introspect-contracts",
            "0x2d26295d6c541d64740e1ae56abc079b82b22c35ab83985ef8bd15dc0f9edfb,0x8b4838140a3cbd36ebe64d4b5aaf56a30cc3753c928a79338bf56c53f506c5",
        ]);

        let introspect = cfg.introspect_contract_addresses().unwrap();
        assert_eq!(introspect.len(), 2);
        assert!(introspect.contains(&Felt::from_hex_unchecked(
            "0x2d26295d6c541d64740e1ae56abc079b82b22c35ab83985ef8bd15dc0f9edfb"
        )));
    }

    #[test]
    fn postgres_storage_defaults_to_shared_postgres_for_tokens() {
        let cfg = Config::parse_from([
            "torii-arcade",
            "--database-url",
            "postgres://torii:torii@localhost:5432/torii",
        ]);

        let token_urls = cfg.token_storage_urls().unwrap();
        assert_eq!(token_urls.0, "postgres://torii:torii@localhost:5432/torii");
        assert_eq!(token_urls.1, token_urls.0);
        assert_eq!(token_urls.2, token_urls.0);
    }

    #[test]
    fn engine_defaults_to_storage_postgres_when_database_url_is_absent() {
        let cfg = Config::parse_from([
            "torii-arcade",
            "--storage-database-url",
            "postgres://torii:torii@localhost:5432/torii",
        ]);

        assert_eq!(
            cfg.engine_database_url(),
            "postgres://torii:torii@localhost:5432/torii"
        );
    }

    #[test]
    fn concurrency_flags_parse() {
        let cfg = Config::parse_from([
            "torii-arcade",
            "--chunk-size",
            "777",
            "--batch-size",
            "8888",
            "--max-prefetch-batches",
            "4",
            "--rpc-parallelism",
            "6",
        ]);

        assert_eq!(cfg.event_chunk_size, 777);
        assert_eq!(cfg.event_block_batch_size, 8888);
        assert_eq!(cfg.max_prefetch_batches, 4);
        assert_eq!(cfg.rpc_parallelism, 6);
    }
}
